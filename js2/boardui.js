...
// ------------------------------------------------------------ hero select
// The family in the cave, before dinner. Whoever you pick is the one who
// will be in the toilet when the knock comes.
class HeroSelectScene {
  constructor() { this.t = 0; this.sel = 0; this.cam = null; }
  enter() { AudioSys.play('home', { fade: 0.8 }); }
  exit() { }
  update(dt) {
    this.t += dt;
    if (Input.pressed('ArrowLeft', 'KeyA')) { this.sel = (this.sel + 3) % 4; AudioSys.sfx('select'); }
    if (Input.pressed('ArrowRight', 'KeyD')) { this.sel = (this.sel + 1) % 4; AudioSys.sfx('select'); }
    for (const k of Input.keys) if (/^Digit[1-4]$/.test(k.code)) { this.sel = +k.code.slice(5) - 1; AudioSys.sfx('select'); }
    if (Input.pressed('Enter', 'Space')) this.go();
    if (Input.pressed('Escape')) Game.go(new TitleScene());
  }
  go() { if (this.gone) return; this.gone = true; AudioSys.sfx('unlock'); Game.newRun(HERO_ORDER[this.sel]); }
  draw() {
    // the cave, warm and lived in
    Gfx.clear('#120c16');
    const ctx = Gfx.ctx;
    if (!this.cam) { this.cam = new Camera(); this.cam.zoom = this.cam.tzoom = VIEW; this.cam.lookAt(HOME.table + 30, GY - 90, true); }
    this.cam.update(Time.dt);
    this.cam.apply(ctx);
    World.home(this.t, {}, this.cam.x - VW / 2);
    if (Settings.lighting !== false) SetLight.run('home', this.t, { fire: 1 }, this.cam.x - VW / 2);
    this.cam.restore(ctx);
    if (Settings.lighting !== false) SetLight.post('home', { fire: 1 });
    Post.ui();
    Gfx.rectA(0, 0, W, H, '#120c16', 0.45);
    Gfx.text('WHO ARE YOU?', W / 2, 18, { color: '#ffe98a', align: 'center', scale: 2.6, outline: true, outlineWidth: 2 });
    Gfx.text('The whole family is home for dinner. Pick who you will play as.', W / 2, 52, { color: '#d6cfe0', align: 'center', scale: 1.1, outline: true });
    const cw = 214, gap = 12, x0 = W / 2 - (cw * 4 + gap * 3) / 2, y0 = 74, ch = 334;
    HERO_ORDER.forEach((id, i) => {
      const Hh = HEROES[id], x = x0 + i * (cw + gap), on = i === this.sel;
      const hov = UI.hovered(x, y0, cw, ch);
      const lift = on ? 6 : hov ? 3 : 0;
      HUD.plate(x, y0 - lift, cw, ch, { hot: on, accent: Hh.color, fill: on ? 'rgba(40,28,48,0.94)' : undefined });
      // the hero, big, bobbing
      const spr = Heroes.spr(id), s = Gfx.spr(spr);
      const sc = s.h > 70 ? 2 : 2;
      const SH = 204;
      Gfx.ctx.save(); Gfx.ctx.beginPath(); Gfx.ctx.rect(x + 4, y0 - lift + 4, cw - 8, SH); Gfx.ctx.clip();
      Gfx.rectA(x + 4, y0 - lift + 4, cw - 8, SH, Hh.dark, 0.35);
      Gfx.shadow(x + cw / 2, y0 - lift + SH, 70, 0.4);
      Gfx.sprite(Heroes.has(id, 'play') && on ? Heroes.spr(id, 'play') : spr, x + cw / 2, y0 - lift + SH + 2 + (on ? Math.abs(Math.sin(this.t * 4)) * -3 : 0), { anchor: 'bc', scale: sc, frame: Math.floor(this.t * (on ? 6 : 2)) });
      Gfx.ctx.restore();
      HUD.text(Hh.name, x + cw / 2, y0 - lift + SH + 8, { color: Hh.color, align: 'center', scale: 2 });
      HUD.text(`${Hh.title}  -  ${Hh.hp} HP`, x + cw / 2, y0 - lift + SH + 30, { color: HUD.C.dim, align: 'center', scale: 1 });
      const mh = Gfx.textWrap(`{y}${Hh.mechanic.name}{/}  ${Hh.mechanic.desc}`, x + 10, y0 - lift + SH + 46, cw - 20, { color: '#d6cfe0', scale: 0.8, lineHeight: 10 });
      Gfx.textWrap(`{y}${Hh.ability.name}{/}  ${Hh.ability.desc}`, x + 10, y0 - lift + SH + 50 + mh, cw - 20, { color: '#d6cfe0', scale: 0.8, lineHeight: 10 });
      UI.hit(x, y0, cw, ch, () => { if (this.sel === i) this.go(); else { this.sel = i; AudioSys.sfx('select'); } });
    });
    // the pick, in full
    const Hh = HEROES[HERO_ORDER[this.sel]];
    const by = y0 + ch + 10;
    HUD.plate(x0, by, cw * 4 + gap * 3 - 210, 76, { gold: false, accent: Hh.color });
    Gfx.textWrap(`{y}${Hh.name}{/}, ${Hh.title.toLowerCase()}. ${Hh.blurb}`, x0 + 12, by + 8, cw * 4 + gap * 3 - 236, { color: '#fffaea', scale: 0.9, lineHeight: 12 });
    Gfx.textWrap(`Plays {y}${Hh.instrument}{/}: ${Hh.instrumentDesc.replace(/\.$/, '')}. Starts with {y}${RELICS[Hh.relic].name}{/}: ${RELICS[Hh.relic].desc}`, x0 + 12, by + 40, cw * 4 + gap * 3 - 236, { color: '#d6cfe0', scale: 0.85, lineHeight: 11 });
    HUD.button(x0 + cw * 4 + gap * 3 - 198, by + 4, 198, 70, `PLAY ${Hh.name}`, () => this.go(), { hot: true, keyHint: 'ENTER', scale: 1.3, accent: Hh.color });
  }
  click() { }
}
